...
const Sidebar = React.memo(({ currentModal,onUsers, onUserSettings, onLogout, user, setDarkUI, setLightUI, setNewDarkUI, setNewLightUI, theme, sidebar, expand, collapse, history, location, newUI}) => {
    ...
    const handleNavigation = (flag) => {
        if(flag === 0) {
            history.push('/componentsNew/');
            setCurrentTab(0);
        } else if(flag === 1) {
            history.push('/componentsNew/dashboard');
            setCurrentTab(1);
        } else if(flag === 2) {
            if(localStorage.getItem('skiddlez-lastBoardId')) {
    ...
    return (
        <div className = {classNames('sidebar', sidebar ? "" : "collapse")}>
            <ul>
                ...
                <li>
                    <UserPopup id = 'user-modal-sidebar' onSettings = {onUserSettings} user = {user} mode = {theme} setDarkUI = {setDarkUI} setLightUI = {setLightUI} setNewDarkUI = {setNewDarkUI} setNewLightUI = {setNewLightUI} onLogout = {onLogout}>
                        <button type = 'button'>
...
Sidebar.propTypes = {
    ...
    setNewDarkUI: PropTypes.func.isRequired,
    setNewLightUI: PropTypes.func.isRequired,
    newUI: PropTypes.func.isRequired,
